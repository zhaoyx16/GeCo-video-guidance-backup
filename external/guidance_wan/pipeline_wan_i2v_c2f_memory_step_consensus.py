...
class WanImageToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def __call__(
        ...
        attn_avg_flow_smooth_sigma: float = 0.0,
        attn_avg_step_consistency_sigma: float = 0.0,
        ...
        Args:
            ...
            attn_avg_flow_smooth_sigma (`float`, defaults to `0.0`):
                Standard deviation, in token units, of a soft local-flow-consistency gate. A value of `0.0`
                disables the gate. Positive values suppress correspondences whose normalized displacement
                is a local spatial outlier, preserving dynamic or occluded regions for the base model.
            attn_avg_step_consistency_sigma (`float`, defaults to `0.0`):
                Standard deviation, in normalized token displacement units, of a soft denoising-step
                consensus gate. A value of `0.0` disables the gate. Positive values suppress a match only
                when its estimated correspondence changes sharply from the previous denoising step.
        ...
        attn_avg_state = {
            ...
            "printed": set(),
            "previous_match_flow": {},
            "previous_match_valid": {},
            "last_step_match_info": {},
        }
        ...
        if attn_avg_flow_smooth_sigma < 0.0:
            raise ValueError("attn_avg_flow_smooth_sigma must be non-negative")
        if attn_avg_step_consistency_sigma < 0.0:
            raise ValueError("attn_avg_step_consistency_sigma must be non-negative")
        ...
        if attn_avg_alpha > 0.0 and attn_avg_layers:
            ...
            def _c2f_match_memory_indices(input_hidden: torch.Tensor, layer_idx: int):
                ...
                best_flow_residual = torch.zeros_like(best_confidence)
                ...
                for lag in range(1, max_lag + 1):
                    ...
                    target_index = torch.arange(spatial_tokens, device=input_hidden.device).view(1, 1, -1)
                    target_y = torch.div(target_index, width_tokens, rounding_mode="floor")
                    target_x = target_index.remainder(width_tokens)
                    source_y = torch.div(source_index, width_tokens, rounding_mode="floor")
                    source_x = source_index.remainder(width_tokens)
                    normalized_flow = torch.stack(
                        [
                            (source_y - target_y).to(torch.float32) / float(lag),
                            (source_x - target_x).to(torch.float32) / float(lag),
                        ],
                        dim=2,
                    ).reshape(batch * frame_pairs, 2, height_tokens, width_tokens)
                    smooth_flow = F.avg_pool2d(
                        normalized_flow, kernel_size=3, stride=1, padding=1, count_include_pad=False
                    )
                    flow_residual = torch.linalg.vector_norm(
                        normalized_flow - smooth_flow, dim=1
                    ).reshape(batch, frame_pairs, spatial_tokens)
                    if attn_avg_flow_smooth_sigma > 0.0:
                        flow_gate = torch.exp(
                            -0.5 * (flow_residual / attn_avg_flow_smooth_sigma).square()
                        )
                        confidence = confidence * flow_gate
                    ...
                    best_flow_residual[:, target_slots] = torch.where(
                        replace, flow_residual, best_flow_residual[:, target_slots]
                    )
                ...
                current_time = torch.arange(
                    1, num_tokens_t, device=input_hidden.device
                ).view(1, num_tokens_t - 1, 1)
                selected_lag = current_time - best_source_time
                target_index = torch.arange(spatial_tokens, device=input_hidden.device).view(1, 1, -1)
                target_y = torch.div(target_index, width_tokens, rounding_mode="floor")
                target_x = target_index.remainder(width_tokens)
                source_y = torch.div(best_source_index, width_tokens, rounding_mode="floor")
                source_x = best_source_index.remainder(width_tokens)
                selected_flow = torch.stack(
                    [
                        (source_y - target_y).to(torch.float32) / selected_lag.to(torch.float32),
                        (source_x - target_x).to(torch.float32) / selected_lag.to(torch.float32),
                    ],
                    dim=-1,
                )
                selected_valid = best_confidence > 0
                previous_flow = attn_avg_state["previous_match_flow"].get(layer_idx)
                previous_valid = attn_avg_state["previous_match_valid"].get(layer_idx)
                if (
                    attn_avg_step_consistency_sigma > 0.0
                    and previous_flow is not None
                    and previous_valid is not None
                    and previous_flow.shape == selected_flow.shape
                ):
                    step_delta = torch.linalg.vector_norm(selected_flow - previous_flow, dim=-1)
                    raw_gate = torch.exp(
                        -0.5 * (step_delta / attn_avg_step_consistency_sigma).square()
                    )
                    step_gate = torch.where(previous_valid, raw_gate, torch.ones_like(raw_gate))
                    best_confidence = best_confidence * step_gate
                attn_avg_state["previous_match_flow"][layer_idx] = selected_flow.detach()
                attn_avg_state["previous_match_valid"][layer_idx] = selected_valid.detach()
                attn_avg_state["last_step_match_info"][layer_idx] = (step_delta, step_gate)
                ...
                return (
                    ...
                    reciprocal,
                    best_flow_residual.reshape(batch, num_tokens_t - 1, height_tokens, width_tokens),
                )
            ...
            def _matched_previous_values(
                ...
                if attn_avg_mode == "c2f_value_residual_anchor":
                    ...
                    (
                        ...
                        reciprocal,
                        flow_residual,
                    ) = _c2f_match_memory_indices(input_grid, layer_idx)
                ...
                def __call__(
                    ...
                    if attn_avg_debug and self.layer_idx == attn_avg_layers[0] and match_similarity is not None:
                        ...
                        if print_key not in attn_avg_state["printed"]:
                            ...
                            flow_residual_info = ""
                            if match_flow_residual is not None and match_confidence is not None:
                                active_flow = match_confidence.reshape_as(match_flow_residual) > 0
                                if active_flow.any():
                                    residual_values = match_flow_residual[active_flow].float()
                                    flow_residual_info = (
                                        f"flow_residual_mean={residual_values.mean().item():.3f} "
                                        f"flow_residual_p90={torch.quantile(residual_values, 0.9).item():.3f} "
                                    )
                            step_consensus_info = ""
                            step_delta, step_gate = attn_avg_state["last_step_match_info"].get(
                                self.layer_idx, (None, None)
                            )
                            if step_delta is not None and step_gate is not None:
                                active_step = match_confidence.reshape_as(step_delta) > 0
                                if active_step.any():
                                    step_consensus_info = (
                                        f"step_delta_mean={step_delta[active_step].float().mean().item():.3f} "
                                        f"step_gate_mean={step_gate[active_step].float().mean().item():.3f} "
                                    )
                            displacement_info = ""
                            ...
                            print(
                                ...
                                f"{flow_residual_info}"
                                f"{step_consensus_info}"
                                f"{reciprocal_info}"
                                f"{displacement_info}"
                                f"{memory_lag_info}"
                            )
