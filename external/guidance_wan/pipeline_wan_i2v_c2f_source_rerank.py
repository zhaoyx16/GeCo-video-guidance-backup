...
class WanImageToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def __call__(
        ...
        c2f_diagnostics: bool = False,
        c2f_diagnostics_sample_size: int = 256,
        c2f_geometry_bundle: dict[str, Any] | None = None,
        c2f_geometry_mode: str = "none",
        c2f_retrieval_policy: str = "V",
        c2f_pair_weighting: str = "hard",
        c2f_source_support_history: int = 3,
        c2f_force_source_score_one: bool = False,
        c2f_geometry_confidence_percentile: float = 20.0,
        c2f_geometry_depth_tolerance: float = 0.15,
        c2f_geometry_max_error_tokens: float = 1.5,
        c2f_geometry_update_callback: Callable[[int, torch.Tensor], dict[str, Any] | None] | None = None,
        c2f_geometry_update_steps: list[int] | None = None,
        c2f_stop_after_geometry_update: bool = False,
    ):
        r"""
        ...
        Args:
            ...
            c2f_diagnostics (`bool`, defaults to `False`):
                Collect read-only C2F correspondence diagnostics in `self._last_c2f_diagnostics`.
                This forensic option does not change matching, blending, or scheduler updates.
            c2f_diagnostics_sample_size (`int`, defaults to `256`):
                Maximum number of deterministically sampled active interventions retained per layer and step.
        ...
        self._last_c2f_diagnostics = None
        self._last_c2f_geometry_diagnostics = None
        prepared_c2f_geometry = None
        ...
        if c2f_diagnostics_sample_size < 0:
            raise ValueError("c2f_diagnostics_sample_size must be non-negative")
        if c2f_diagnostics and attn_avg_mode != "c2f_value_residual_memory":
            raise ValueError("c2f_diagnostics currently requires attn_avg_mode='c2f_value_residual_memory'")
        if c2f_geometry_mode not in {"none", "rerank", "uniform_norm_control"}:
            raise ValueError(
                "c2f_geometry_mode must be one of: none, rerank, uniform_norm_control"
            )
        if c2f_retrieval_policy not in {"V", "P", "S"}:
            raise ValueError("c2f_retrieval_policy must be one of: V, P, S")
        if c2f_pair_weighting not in {"hard", "soft"}:
            raise ValueError("c2f_pair_weighting must be one of: hard, soft")
        if c2f_source_support_history < 1:
            raise ValueError("c2f_source_support_history must be positive")
        geometry_update_steps = tuple(int(step) for step in (c2f_geometry_update_steps or ()))
        if geometry_update_steps != tuple(sorted(set(geometry_update_steps))):
            raise ValueError("c2f_geometry_update_steps must be sorted and unique")
        if any(step < 0 or step >= num_inference_steps for step in geometry_update_steps):
            raise ValueError("c2f_geometry_update_steps contains an invalid sampling-step index")
        if bool(geometry_update_steps) != (c2f_geometry_update_callback is not None):
            raise ValueError(
                "c2f_geometry_update_callback and non-empty c2f_geometry_update_steps must be provided together"
            )
        if c2f_stop_after_geometry_update:
            if c2f_geometry_update_callback is None:
                raise ValueError("c2f_stop_after_geometry_update requires a geometry update callback")
            if output_type != "latent":
                raise ValueError("c2f_stop_after_geometry_update requires output_type='latent'")
        if c2f_geometry_mode == "uniform_norm_control" and c2f_retrieval_policy == "S":
            raise ValueError("the frozen matrix defines uniform controls only for V and P")
        if c2f_geometry_mode != "none":
            if attn_avg_mode != "c2f_value_residual_memory":
                raise ValueError("C2F geometry validation requires attn_avg_mode='c2f_value_residual_memory'")
            if c2f_geometry_bundle is None:
                raise ValueError("an initial c2f_geometry_bundle is required when geometry validation is enabled")
            if not 0.0 <= c2f_geometry_confidence_percentile <= 100.0:
                raise ValueError("c2f_geometry_confidence_percentile must lie in [0, 100]")
            if c2f_geometry_depth_tolerance < 0.0:
                raise ValueError("c2f_geometry_depth_tolerance must be non-negative")
            if c2f_geometry_max_error_tokens < 0.0:
                raise ValueError("c2f_geometry_max_error_tokens must be non-negative")
        ...
        if c2f_diagnostics:
            self._last_c2f_diagnostics = {
                "schema": "wan-c2f-intervention-diagnostics-v1",
                "method": {
                    "attn_avg_alpha": attn_avg_alpha,
                    "attn_avg_layers": list(attn_avg_layers or []),
                    "attn_avg_mode": attn_avg_mode,
                    "attn_avg_start": attn_avg_start,
                    "attn_avg_end": attn_avg_end,
                    "attn_avg_match_radius": attn_avg_match_radius,
                    "attn_avg_match_confidence": attn_avg_match_confidence,
                    "attn_avg_match_mutual": attn_avg_match_mutual,
                    "attn_avg_descriptor_dim": attn_avg_descriptor_dim,
                    "attn_avg_coarse_factor": attn_avg_coarse_factor,
                    "attn_avg_memory_lookback": attn_avg_memory_lookback,
                    "attn_avg_cond_only": attn_avg_cond_only,
                    "attn_avg_preserve_first_frame": attn_avg_preserve_first_frame,
                },
                "sample_size_per_layer_step": c2f_diagnostics_sample_size,
                "records": [],
            }
        if (attn_avg_alpha > 0.0 or c2f_diagnostics) and attn_avg_layers:
            ...
            if c2f_geometry_mode != "none":
                prepared_c2f_geometry = prepare_geometry_bundle(
                    c2f_geometry_bundle,
                    device=latents.device,
                    expected_frames=token_grid[0],
                    confidence_percentile=c2f_geometry_confidence_percentile,
                )
                self._last_c2f_geometry_diagnostics = {
                    "schema": "wan-c2f-source-rerank-intervention-v1",
                    "mode": c2f_geometry_mode,
                    "retrieval_policy": c2f_retrieval_policy,
                    "pair_weighting": c2f_pair_weighting,
                    "source_support_history": c2f_source_support_history,
                    "force_source_score_one": c2f_force_source_score_one,
                    "confidence_percentile": c2f_geometry_confidence_percentile,
                    "depth_tolerance": c2f_geometry_depth_tolerance,
                    "max_error_tokens": c2f_geometry_max_error_tokens,
                    "token_grid_thw": list(token_grid),
                    "geometry_image_size_hw": list(prepared_c2f_geometry["image_size_hw"]),
                    "geometry_update_steps": list(geometry_update_steps),
                    "geometry_updates": [],
                    "records": [],
                }
            ...
            def _c2f_match_memory_indices(input_hidden: torch.Tensor):
                ...
                need_candidate_bank = c2f_diagnostics or c2f_geometry_mode != "none"

                match_diagnostics = None
                if need_candidate_bank:
                    diagnostic_shape = (batch, num_tokens_t - 1, spatial_tokens, max_lag)
                    candidate_source_index = torch.full(
                        diagnostic_shape, -1, device=input_hidden.device, dtype=torch.long
                    )
                    candidate_source_time = torch.full_like(candidate_source_index, -1)
                    candidate_raw_confidence = torch.zeros(
                        diagnostic_shape, device=input_hidden.device, dtype=torch.float32
                    )
                    candidate_confidence = torch.zeros_like(candidate_raw_confidence)
                    candidate_similarity = torch.full_like(candidate_raw_confidence, -1.0)
                    candidate_fine_margin = torch.zeros_like(candidate_raw_confidence)
                    candidate_coarse_margin = torch.zeros_like(candidate_raw_confidence)
                    candidate_coarse_cycle_error = torch.zeros_like(candidate_raw_confidence)
                    candidate_reciprocal = torch.zeros(
                        diagnostic_shape, device=input_hidden.device, dtype=torch.bool
                    )
                    candidate_valid = torch.zeros_like(candidate_reciprocal)
                ...
                for lag in range(1, max_lag + 1):
                    ...
                    coarse_best_similarity, source_coarse_index = coarse_similarity.max(dim=-1)
                    coarse_margin = None
                    if need_candidate_bank:
                        if coarse_tokens > 1:
                            coarse_top_two = torch.topk(coarse_similarity, k=2, dim=-1).values
                            coarse_margin = coarse_top_two[..., 0] - coarse_top_two[..., 1]
                        else:
                            coarse_margin = coarse_best_similarity

                    reciprocal_coarse = None
                    coarse_cycle_error = None
                    if attn_avg_match_mutual:
                        reverse_best_index = coarse_similarity.argmax(dim=1)
                        returned_target = reverse_best_index.gather(1, source_coarse_index)
                        current_index = torch.arange(coarse_tokens, device=input_hidden.device).view(1, -1)
                        ...
                        if need_candidate_bank:
                            returned_y = torch.div(returned_target, width_coarse, rounding_mode="floor")
                            returned_x = returned_target.remainder(width_coarse)
                            current_y = torch.div(current_index, width_coarse, rounding_mode="floor")
                            current_x = current_index.remainder(width_coarse)
                            coarse_cycle_error = torch.sqrt(
                                (returned_y - current_y).float().square()
                                + (returned_x - current_x).float().square()
                            ).reshape(batch, frame_pairs, height_coarse, width_coarse)
                    ...
                    fine_margin = None
                    if need_candidate_bank:
                        if candidates_per_token > 1:
                            fine_top_two = torch.topk(fine_similarity, k=2, dim=-1).values
                            fine_margin = fine_top_two[..., 0] - fine_top_two[..., 1]
                        else:
                            fine_margin = fine_best_similarity
                    ...
                    if need_candidate_bank:
                        lag_slot = lag - 1
                        candidate_source_index[:, target_slots, :, lag_slot] = source_index
                        candidate_source_time[:, target_slots, :, lag_slot] = source_time
                        candidate_raw_confidence[:, target_slots, :, lag_slot] = raw_confidence
                        candidate_confidence[:, target_slots, :, lag_slot] = confidence
                        candidate_similarity[:, target_slots, :, lag_slot] = fine_best_similarity
                        candidate_fine_margin[:, target_slots, :, lag_slot] = fine_margin
                        coarse_margin_fine = coarse_margin.reshape(
                            batch, frame_pairs, height_coarse, width_coarse
                        ).repeat_interleave(factor, dim=2).repeat_interleave(factor, dim=3)
                        candidate_coarse_margin[:, target_slots, :, lag_slot] = coarse_margin_fine.reshape(
                            batch, frame_pairs, spatial_tokens
                        )
                        if reciprocal_fine is None:
                            candidate_reciprocal[:, target_slots, :, lag_slot] = True
                        else:
                            candidate_reciprocal[:, target_slots, :, lag_slot] = reciprocal_fine
                        if coarse_cycle_error is not None:
                            coarse_cycle_error_fine = coarse_cycle_error.repeat_interleave(
                                factor, dim=2
                            ).repeat_interleave(factor, dim=3)
                            candidate_coarse_cycle_error[:, target_slots, :, lag_slot] = (
                                coarse_cycle_error_fine.reshape(batch, frame_pairs, spatial_tokens)
                            )
                        candidate_valid[:, target_slots, :, lag_slot] = True
                ...
                if need_candidate_bank:
                    candidate_path_cycle_error = torch.zeros_like(candidate_confidence)
                    lag_one_map = candidate_source_index[..., 0]
                    for lag in range(2, max_lag + 1):
                        target_slots = torch.arange(lag - 1, num_tokens_t - 1, device=input_hidden.device)
                        chained_source = lag_one_map[:, target_slots]
                        for hop in range(2, lag + 1):
                            previous_map = lag_one_map[:, target_slots - (hop - 1)]
                            chained_source = previous_map.gather(2, chained_source)
                        direct_source = candidate_source_index[:, target_slots, :, lag - 1]
                        direct_y = torch.div(direct_source, width_tokens, rounding_mode="floor")
                        direct_x = direct_source.remainder(width_tokens)
                        chained_y = torch.div(chained_source, width_tokens, rounding_mode="floor")
                        chained_x = chained_source.remainder(width_tokens)
                        candidate_path_cycle_error[:, target_slots, :, lag - 1] = torch.sqrt(
                            (direct_y - chained_y).float().square()
                            + (direct_x - chained_x).float().square()
                        )

                    current_index = torch.arange(spatial_tokens, device=input_hidden.device).view(1, 1, -1, 1)
                    current_y = torch.div(current_index, width_tokens, rounding_mode="floor")
                    current_x = current_index.remainder(width_tokens)
                    safe_source_index = candidate_source_index.clamp_min(0)
                    source_y = torch.div(safe_source_index, width_tokens, rounding_mode="floor")
                    source_x = safe_source_index.remainder(width_tokens)
                    lag_values = torch.arange(
                        1, max_lag + 1, device=input_hidden.device, dtype=torch.float32
                    ).view(1, 1, 1, -1)
                    velocity_y = (current_y - source_y).float() / lag_values
                    velocity_x = (current_x - source_x).float() / lag_values
                    agreement_weights = candidate_raw_confidence * candidate_valid.to(torch.float32)
                    agreement_weight_sum = agreement_weights.sum(dim=-1).clamp_min(1e-8)
                    mean_velocity_y = (agreement_weights * velocity_y).sum(dim=-1) / agreement_weight_sum
                    mean_velocity_x = (agreement_weights * velocity_x).sum(dim=-1) / agreement_weight_sum
                    velocity_square_error = (
                        (velocity_y - mean_velocity_y.unsqueeze(-1)).square()
                        + (velocity_x - mean_velocity_x.unsqueeze(-1)).square()
                    )
                    velocity_rms = torch.sqrt(
                        (agreement_weights * velocity_square_error).sum(dim=-1) / agreement_weight_sum
                    )

                    history_top_two = torch.topk(
                        candidate_confidence, k=min(2, max_lag), dim=-1
                    ).values
                    history_confidence_margin = (
                        history_top_two[..., 0] - history_top_two[..., 1]
                        if max_lag > 1
                        else history_top_two[..., 0]
                    )
                    current_time = torch.arange(1, num_tokens_t, device=input_hidden.device).view(1, -1, 1)
                    selected_lag = (current_time - best_source_time).clamp(1, max_lag)
                    selected_lag_index = (selected_lag - 1).unsqueeze(-1)

                    def _selected(candidate: torch.Tensor) -> torch.Tensor:
                        return candidate.gather(-1, selected_lag_index).squeeze(-1)

                    source_lookup = (
                        (best_source_time - 1).clamp_min(0) * spatial_tokens + best_source_index
                    )
                    source_previous_confidence = best_confidence.reshape(batch, -1).gather(
                        1, source_lookup.reshape(batch, -1)
                    ).reshape_as(best_confidence)
                    source_previous_similarity = best_similarity.reshape(batch, -1).gather(
                        1, source_lookup.reshape(batch, -1)
                    ).reshape_as(best_similarity)
                    source_previous_reciprocal = best_reciprocal.reshape(batch, -1).gather(
                        1, source_lookup.reshape(batch, -1)
                    ).reshape_as(best_reciprocal)
                    source_is_conditioning = best_source_time.eq(0)
                    source_previous_confidence = torch.where(
                        source_is_conditioning, torch.ones_like(source_previous_confidence), source_previous_confidence
                    )
                    source_previous_similarity = torch.where(
                        source_is_conditioning, torch.ones_like(source_previous_similarity), source_previous_similarity
                    )
                    source_previous_reciprocal = torch.where(
                        source_is_conditioning,
                        torch.ones_like(source_previous_reciprocal),
                        source_previous_reciprocal,
                    )
                    ...
                    match_diagnostics = {
                        ...
                        "candidate_raw_confidence": candidate_raw_confidence,
                        "candidate_confidence": candidate_confidence,
                        "candidate_similarity": candidate_similarity,
                        "candidate_fine_margin": candidate_fine_margin,
                        "candidate_coarse_margin": candidate_coarse_margin,
                        "candidate_reciprocal": candidate_reciprocal,
                        "candidate_coarse_cycle_error": candidate_coarse_cycle_error,
                        "candidate_path_cycle_error": candidate_path_cycle_error,
                        "selected_fine_margin": _selected(candidate_fine_margin),
                        "selected_coarse_margin": _selected(candidate_coarse_margin),
                        "selected_coarse_cycle_error": _selected(candidate_coarse_cycle_error),
                        "selected_path_cycle_error": _selected(candidate_path_cycle_error),
                        "selected_lag": selected_lag,
                        "history_confidence_margin": history_confidence_margin,
                        "raw_candidate_count": (candidate_raw_confidence > 0).sum(dim=-1),
                        "gated_candidate_count": (candidate_confidence > 0).sum(dim=-1),
                        "velocity_rms": velocity_rms,
                        "source_previous_confidence": source_previous_confidence,
                        "source_previous_similarity": source_previous_similarity,
                        "source_previous_reciprocal": source_previous_reciprocal,
                        "source_is_conditioning": source_is_conditioning,
                    }
                return (
                    best_source_index,
                    best_source_time,
            ...
            def _matched_previous_values(
                ...
                if attn_avg_mode == "c2f_value_residual_memory":
                    if c2f_geometry_mode != "none":
                        if match_diagnostics is None or prepared_c2f_geometry is None:
                            raise RuntimeError("source reranking requires the candidate bank and prepared geometry")
                        candidate_source_index = match_diagnostics["candidate_source_index"]
                        candidate_source_time = match_diagnostics["candidate_source_time"]
                        candidate_confidence = match_diagnostics["candidate_confidence"]
                        candidate_valid = match_diagnostics["candidate_valid"]
                        pair_weight, pair_info = candidate_pairwise_geometry(
                            candidate_source_index,
                            candidate_source_time,
                            candidate_confidence,
                            candidate_valid,
                            token_grid,
                            prepared_c2f_geometry,
                            weighting=c2f_pair_weighting,
                            depth_tolerance=c2f_geometry_depth_tolerance,
                            max_error_tokens=c2f_geometry_max_error_tokens,
                        )
                        source_score, source_info = candidate_source_support(
                            candidate_source_index,
                            candidate_source_time,
                            candidate_confidence,
                            candidate_valid,
                            token_grid,
                            prepared_c2f_geometry,
                            history=c2f_source_support_history,
                            depth_tolerance=c2f_geometry_depth_tolerance,
                            max_error_tokens=c2f_geometry_max_error_tokens,
                        )
                        if c2f_force_source_score_one:
                        ...
                        base_selection = selections["V"]
                        target_selection = selections[c2f_retrieval_policy]
                        actual_selection = (
                            target_selection if c2f_geometry_mode == "rerank" else base_selection
                        )

                        value_flat = value_grid.reshape(
                            batch_size, num_tokens_t * spatial_tokens, value_channels
                        )

                        def _values_for(selection: dict[str, torch.Tensor]) -> torch.Tensor:
                            flat_index = (
                                selection["source_time"] * spatial_tokens + selection["source_index"]
                            )
                            return value_flat.gather(
                                1,
                                flat_index.reshape(batch_size, -1)
                                .unsqueeze(-1)
                                .expand(-1, -1, value_channels),
                            ).reshape(batch_size, num_tokens_t - 1, spatial_tokens, value_channels)

                        matched_values = _values_for(actual_selection)
                        target_matched_values = _values_for(target_selection)
                        choice = actual_selection["choice"]
                        confidence = actual_selection["confidence"].reshape(
                            batch_size, num_tokens_t - 1, height_tokens, width_tokens, 1
                        )
                        best_similarity = match_diagnostics["candidate_similarity"].gather(
                            -1, choice.unsqueeze(-1)
                        ).squeeze(-1).reshape(
                            batch_size, num_tokens_t - 1, height_tokens, width_tokens
                        )
                        reciprocal = match_diagnostics["candidate_reciprocal"].gather(
                            -1, choice.unsqueeze(-1)
                        ).squeeze(-1).reshape(
                            batch_size, num_tokens_t - 1, height_tokens, width_tokens
                        )
                        source_index = actual_selection["source_index"]
                        source_time = actual_selection["source_time"]
                        geometry_selection = {
                            "selections": selections,
                            "actual": actual_selection,
                            "base": base_selection,
                            "target": target_selection,
                            "target_matched_values": target_matched_values,
                        ...
                        matched_values = value_grid.reshape(
                            batch_size, num_tokens_t * spatial_tokens, value_channels
                        ).gather(
                            1,
            ...
            def _diagnostic_stats(values: torch.Tensor, mask: torch.Tensor | None = None) -> dict[str, float | int | None]:
                ...
                quantiles = torch.quantile(
                    values, torch.tensor([0.1, 0.5, 0.9], device=values.device, dtype=values.dtype)
                )
                return {
                    "count": count,
            ...
            def _record_c2f_diagnostics(
                layer_idx: int,
                attended_grid: torch.Tensor,
                matched_value_grid: torch.Tensor,
                match_confidence: torch.Tensor,
                match_similarity: torch.Tensor,
                match_source_index: torch.Tensor,
                match_source_time: torch.Tensor,
                match_diagnostics: dict[str, torch.Tensor],
            ) -> None:
                if not c2f_diagnostics or self._last_c2f_diagnostics is None:
                    return

                num_tokens_t, height_tokens, width_tokens = token_grid
                spatial_tokens = height_tokens * width_tokens
                confidence = match_confidence.reshape(batch_size, num_tokens_t - 1, spatial_tokens).detach().float()
                similarity = match_similarity.reshape_as(confidence).detach().float()
                active = confidence > 0
                active_count = int(active.sum().item())
                target_count = int(active.numel())

                current_context = attended_grid[:, 1:].detach().float()
                source_value = matched_value_grid.detach().float()
                residual_norm = torch.linalg.vector_norm(
                    (source_value - current_context).flatten(-2), dim=-1
                )
                current_norm = torch.linalg.vector_norm(current_context.flatten(-2), dim=-1)
                source_norm = torch.linalg.vector_norm(source_value.flatten(-2), dim=-1)
                relative_residual = residual_norm / current_norm.clamp_min(1e-8)
                relative_update = attn_avg_alpha * confidence * relative_residual
                source_to_context_norm_ratio = source_norm / current_norm.clamp_min(1e-8)

                current_index = torch.arange(spatial_tokens, device=confidence.device).view(1, 1, -1)
                current_y = torch.div(current_index, width_tokens, rounding_mode="floor")
                current_x = current_index.remainder(width_tokens)
                ...
                displacement = torch.sqrt(
                    (source_y - current_y).float().square() + (source_x - current_x).float().square()
                )

                source_previous_confidence = match_diagnostics["source_previous_confidence"].detach().float()
                source_is_conditioning = match_diagnostics["source_is_conditioning"].detach()
                source_unreliable = (~source_is_conditioning) & source_previous_confidence.le(0)
                active_denominator = max(active_count, 1)

                selected_lag = match_diagnostics["selected_lag"].detach()
                lag_histogram = {
                    str(lag): int((active & selected_lag.eq(lag)).sum().item())
                    for lag in range(1, min(attn_avg_memory_lookback, num_tokens_t - 1) + 1)
                }

                selected_fields = {
                    "confidence": confidence,
                    "similarity": similarity,
                    "fine_margin": match_diagnostics["selected_fine_margin"],
                    "coarse_margin": match_diagnostics["selected_coarse_margin"],
                    "coarse_cycle_error": match_diagnostics["selected_coarse_cycle_error"],
                    "path_cycle_error": match_diagnostics["selected_path_cycle_error"],
                    "velocity_rms": match_diagnostics["velocity_rms"],
                    "history_confidence_margin": match_diagnostics["history_confidence_margin"],
                    "raw_candidate_count": match_diagnostics["raw_candidate_count"],
                    "gated_candidate_count": match_diagnostics["gated_candidate_count"],
                    "source_previous_confidence": source_previous_confidence,
                    "source_previous_similarity": match_diagnostics["source_previous_similarity"],
                    "relative_value_residual": relative_residual,
                    "relative_update": relative_update,
                    "source_to_context_norm_ratio": source_to_context_norm_ratio,
                    "spatial_displacement": displacement,
                    "selected_lag": selected_lag,
                }

                lag_records = []
                max_lag = match_diagnostics["candidate_valid"].shape[-1]
                for lag_index in range(max_lag):
                    valid = match_diagnostics["candidate_valid"][..., lag_index]
                    raw_active = match_diagnostics["candidate_raw_confidence"][..., lag_index] > 0
                    gated_active = match_diagnostics["candidate_confidence"][..., lag_index] > 0
                    lag_records.append(
                        {
                            "lag": lag_index + 1,
                            "valid_count": int(valid.sum().item()),
                            "raw_active_fraction": float((raw_active & valid).sum().item() / max(int(valid.sum().item()), 1)),
                            "gated_active_fraction": float(
                                (gated_active & valid).sum().item() / max(int(valid.sum().item()), 1)
                            ),
                            "reciprocal_fraction": float(
                                (match_diagnostics["candidate_reciprocal"][..., lag_index] & valid).sum().item()
                                / max(int(valid.sum().item()), 1)
                            ),
                            "similarity": _diagnostic_stats(
                                match_diagnostics["candidate_similarity"][..., lag_index], valid
                            ),
                            "fine_margin": _diagnostic_stats(
                                match_diagnostics["candidate_fine_margin"][..., lag_index], valid
                            ),
                            "coarse_margin": _diagnostic_stats(
                                match_diagnostics["candidate_coarse_margin"][..., lag_index], valid
                            ),
                            "coarse_cycle_error": _diagnostic_stats(
                                match_diagnostics["candidate_coarse_cycle_error"][..., lag_index], valid
                            ),
                            "path_cycle_error": _diagnostic_stats(
                                match_diagnostics["candidate_path_cycle_error"][..., lag_index], valid
                            ),
                        }
                    )

                sample_columns: dict[str, list] = {}
                if c2f_diagnostics_sample_size > 0 and active_count > 0:
                    active_flat = torch.nonzero(active.reshape(-1), as_tuple=False).squeeze(1)
                    keep = min(c2f_diagnostics_sample_size, active_count)
                    positions = (
                        torch.arange(keep, device=active.device, dtype=torch.long) * active_count // keep
                    )
                    sample_flat = active_flat[positions]
                    per_batch = (num_tokens_t - 1) * spatial_tokens
                    sample_batch = torch.div(sample_flat, per_batch, rounding_mode="floor")
                    sample_remainder = sample_flat.remainder(per_batch)
                    sample_target_slot = torch.div(sample_remainder, spatial_tokens, rounding_mode="floor")
                    sample_target_index = sample_remainder.remainder(spatial_tokens)
                    sample_source_time = match_source_time.reshape(-1)[sample_flat]
                    sample_source_index = match_source_index.reshape(-1)[sample_flat]
                    integer_columns = {
                        "batch": sample_batch,
                        "target_time": sample_target_slot + 1,
                        "target_y": torch.div(sample_target_index, width_tokens, rounding_mode="floor"),
                        "target_x": sample_target_index.remainder(width_tokens),
                        "source_time": sample_source_time,
                        "source_y": torch.div(sample_source_index, width_tokens, rounding_mode="floor"),
                        "source_x": sample_source_index.remainder(width_tokens),
                    }
                    for name, values in integer_columns.items():
                        sample_columns[name] = values.detach().cpu().tolist()
                    for name, values in selected_fields.items():
                        sample_columns[name] = values.detach().reshape(-1)[sample_flat].float().cpu().tolist()
                    sample_columns["source_is_conditioning"] = (
                        source_is_conditioning.reshape(-1)[sample_flat].cpu().tolist()
                    )

                self._last_c2f_diagnostics["records"].append(
                    {
                        "step": int(attn_avg_state["step"]),
                        "layer": int(layer_idx),
                        "branch": attn_avg_state["branch"],
                        "token_grid": [int(value) for value in token_grid],
                        "target_token_count": target_count,
                        "active_count": active_count,
                        "active_fraction": float(active_count / max(target_count, 1)),
                        "source_unreliable_active_fraction": float(
                            (source_unreliable & active).sum().item() / active_denominator
                        ),
                        "conditioning_source_active_fraction": float(
                            (source_is_conditioning & active).sum().item() / active_denominator
                        ),
                        "selected_lag_histogram": lag_histogram,
                        "selected": {
                            name: _diagnostic_stats(values, active) for name, values in selected_fields.items()
                        },
                        "candidate_by_lag": lag_records,
                    ...
                    if matched_values is not None and match_confidence is not None:
                        ...
                        if c2f_geometry_mode != "none":
                            if geometry_selection is None:
                                raise RuntimeError("source reranking selection metadata is missing")
                            actual_selection = geometry_selection["actual"]
                            target_selection = geometry_selection["target"]
                            actual_weight = actual_selection["update_weight"].reshape(
                                batch_size, num_tokens_t - 1, spatial_tokens, 1, 1
                            )
                            residual = matched_value_grid - attended_grid[:, 1:]
                            uniform_scale = torch.ones((), device=residual.device, dtype=torch.float32)
                            update_norms = None
                            if c2f_geometry_mode == "rerank":
                                blend = attn_avg_alpha * actual_weight
                                actual_update_norm = torch.linalg.vector_norm(
                                    residual.float() * blend.float()
                                )
                                target_update_norm = actual_update_norm
                                norm_error = torch.zeros_like(actual_update_norm)
                            else:
                                target_value_grid = geometry_selection["target_matched_values"].unflatten(
                                    -1, (attn.heads, -1)
                                )
                                target_residual = target_value_grid - attended_grid[:, 1:]
                                base_weight = geometry_selection["base"]["confidence"]
                                target_weight = target_selection["update_weight"]
                                uniform_scale, update_norms = uniform_scale_to_target_update(
                                    residual,
                                    base_weight,
                                    target_residual,
                                    target_weight,
                                )
                                blend = base_blend * uniform_scale
                                actual_update_norm = attn_avg_alpha * update_norms["actual_update_norm"]
                                target_update_norm = attn_avg_alpha * update_norms["target_update_norm"]
                                norm_error = update_norms["norm_match_relative_error"]

                            selections = geometry_selection["selections"]
                            candidate_active = (
                                match_diagnostics["candidate_valid"]
                                & (match_diagnostics["candidate_confidence"] > 0)
                            )
                            evidence = geometry_selection["source_info"]
                            candidate_decidable = (
                                evidence["support_count"] + evidence["conflict_count"]
                            ) > 0
                            p_selection = selections["P"]
                            s_selection = selections["S"]
                            p_s_comparable = p_selection["valid"] & s_selection["valid"]
                            p_s_changed = p_s_comparable & (
                                (p_selection["source_time"] != s_selection["source_time"])
                                | (p_selection["source_index"] != s_selection["source_index"])
                            )
                            base_active = geometry_selection["base"]["confidence"] > 0
                            update_active = actual_selection["update_weight"] > 0
                            active_count = int(base_active.sum().item())
                            ...
                            record = {
                                ...
                                "branch": attn_avg_state["branch"],
                                "feature_top1_active": active_count,
                                "actual_update_active": update_count,
                                "survival_fraction": update_count / active_count if active_count else 0.0,
                                "candidate_count": int(candidate_active.sum().item()),
                                "candidate_evidence_coverage": float(
                                    (candidate_active & candidate_decidable).sum().item()
                                    / max(int(candidate_active.sum().item()), 1)
                                ),
                                "candidate_support_views": int(
                                    (evidence["support"] & candidate_active.unsqueeze(-1)).sum().item()
                                ),
                                "candidate_conflict_views": int(
                                    (evidence["conflict"] & candidate_active.unsqueeze(-1)).sum().item()
                                ),
                                "candidate_unknown_views": int(
                                    (evidence["unknown"] & candidate_active.unsqueeze(-1)).sum().item()
                                ),
                                "source_score": _diagnostic_stats(
                                    geometry_selection["source_score"], candidate_active
                                ),
                                "selected_source_score": _diagnostic_stats(
                                    actual_selection["source_score"], update_active
                                ),
                                "selected_pair_weight": _diagnostic_stats(
                                    actual_selection["pair_weight"], base_active
                                ),
                                "selected_time": _diagnostic_stats(
                                    actual_selection["source_time"].float(), base_active
                                ),
                                "p_s_comparable": int(p_s_comparable.sum().item()),
                                "p_s_changed": int(p_s_changed.sum().item()),
                                "p_s_changed_fraction": float(
                                    p_s_changed.sum().item() / max(int(p_s_comparable.sum().item()), 1)
                                ),
                                "uniform_scale": float(uniform_scale.item()),
                                "target_update_norm": float(target_update_norm.item()),
                                "actual_update_norm": float(actual_update_norm.item()),
                                "norm_match_relative_error": float(norm_error.item()),
                            }
                            if update_norms is not None:
                                record["uniform_unconstrained_scale"] = float(
                                    update_norms["unconstrained_scale"].item()
                                )
                                record["uniform_base_update_norm"] = float(
                                    (attn_avg_alpha * update_norms["base_update_norm"]).item()
                                )
                            self.geometry_diagnostics["records"].append(record)
                        if attn_avg_alpha > 0.0:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                stop_after_geometry_update = False
                if c2f_geometry_update_callback is not None and i in geometry_update_steps:
                    if self.scheduler.step_index is None:
                        self.scheduler._init_step_index(t)
                    x0_geometry = self.scheduler.convert_model_output(
                        noise_pred.float().detach(), sample=latents.float()
                    )
                    if self.config.expand_timesteps:
                        x0_geometry = (
                            (1 - first_frame_mask.float()) * condition.float()
                            + first_frame_mask.float() * x0_geometry.float()
                        )
                    updated_bundle = c2f_geometry_update_callback(i, x0_geometry.detach())
                    del x0_geometry
                    if updated_bundle is not None:
                        ...
                        if self._last_c2f_geometry_diagnostics is not None:
                            self._last_c2f_geometry_diagnostics["geometry_updates"].append(
                                {
                                    "step": int(i),
                                    "image_size_hw": list(prepared_c2f_geometry["image_size_hw"]),
                                }
                            )
                    stop_after_geometry_update = c2f_stop_after_geometry_update

                if stop_after_geometry_update:
                    progress_bar.update()
                    break
