...
class WanImageToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def __call__(
        ...
        if attn_avg_mode not in {
            "global", "local", "anchor", "match_prev", "c2f_match_prev", "query_match_prev",
            "key_match_prev", "kv_match_prev", "value_residual_prev", "c2f_value_residual_prev",
            "c2f_value_residual_anchor", "c2f_value_residual_memory", "c2f_qk_value_residual_memory",
        }:
            raise ValueError(
        ...
        if attn_avg_alpha > 0.0 and attn_avg_layers:
            ...
            def _reduced_qk_descriptors(
                normalized_query: torch.Tensor,
                normalized_key: torch.Tensor,
            ) -> tuple[torch.Tensor, torch.Tensor]:
                """Approximate native Q/K affinity with evenly spread attention dimensions."""
                batch, sequence_length, channels = normalized_query.shape
                num_tokens_t, height_tokens, width_tokens = token_grid
                expected_sequence_length = num_tokens_t * height_tokens * width_tokens
                if sequence_length != expected_sequence_length or normalized_key.shape != normalized_query.shape:
                    raise ValueError("Q/K descriptor shape does not match the Wan token grid")
                descriptor_dim = min(attn_avg_descriptor_dim, channels)
                descriptor_indices = torch.linspace(
                    0, channels - 1, descriptor_dim, device=normalized_query.device
                ).round().to(torch.long)
                query_descriptor = normalized_query.index_select(-1, descriptor_indices).reshape(
                    batch, num_tokens_t, height_tokens, width_tokens, descriptor_dim
                )
                key_descriptor = normalized_key.index_select(-1, descriptor_indices).reshape(
                    batch, num_tokens_t, height_tokens, width_tokens, descriptor_dim
                )
                return (
                    F.normalize(query_descriptor.float(), dim=-1, eps=1e-6),
                    F.normalize(key_descriptor.float(), dim=-1, eps=1e-6),
                )
            ...
            def _c2f_match_memory_indices(
                input_hidden: torch.Tensor,
                query_descriptors: torch.Tensor | None = None,
                key_descriptors: torch.Tensor | None = None,
            ):
                ...
                if (query_descriptors is None) != (key_descriptors is None):
                    raise ValueError("Pass both query and key descriptors, or neither")
                asymmetric_qk = query_descriptors is not None
                if query_descriptors is None:
                    descriptors = _reduced_descriptors(input_hidden)
                    query_descriptors = descriptors
                    key_descriptors = descriptors
                if query_descriptors.shape[:-1] != input_hidden.shape[:-1] or key_descriptors.shape != query_descriptors.shape:
                    raise ValueError("Correspondence descriptors must match the Wan token grid")
                descriptor_dim = query_descriptors.shape[-1]
                height_coarse, width_coarse = height_tokens // factor, width_tokens // factor
                coarse_tokens = height_coarse * width_coarse
                spatial_tokens = height_tokens * width_tokens

                query_descriptor_4d = query_descriptors.permute(0, 1, 4, 2, 3).reshape(
                    batch * num_tokens_t, descriptor_dim, height_tokens, width_tokens
                )
                key_descriptor_4d = key_descriptors.permute(0, 1, 4, 2, 3).reshape(
                    batch * num_tokens_t, descriptor_dim, height_tokens, width_tokens
                )
                coarse_query = F.avg_pool2d(query_descriptor_4d, kernel_size=factor, stride=factor)
                coarse_key = F.avg_pool2d(key_descriptor_4d, kernel_size=factor, stride=factor)
                coarse_query = F.normalize(
                    coarse_query.reshape(batch, num_tokens_t, descriptor_dim, height_coarse, width_coarse)
                    .permute(0, 1, 3, 4, 2),
                    dim=-1,
                    eps=1e-6,
                )
                coarse_key = F.normalize(
                    coarse_key.reshape(batch, num_tokens_t, descriptor_dim, height_coarse, width_coarse)
                    .permute(0, 1, 3, 4, 2),
                    dim=-1,
                    eps=1e-6,
                )
                ...
                for lag in range(1, max_lag + 1):
                    ...
                    reference_coarse = coarse_key[:, :-lag].reshape(frame_pairs * batch, coarse_tokens, descriptor_dim)
                    current_coarse = coarse_query[:, lag:].reshape(frame_pairs * batch, coarse_tokens, descriptor_dim)
                    ...
                    if attn_avg_match_mutual:
                        if asymmetric_qk:
                            reverse_similarity = torch.bmm(
                                coarse_query[:, :-lag].reshape(frame_pairs * batch, coarse_tokens, descriptor_dim),
                                coarse_key[:, lag:].reshape(frame_pairs * batch, coarse_tokens, descriptor_dim).transpose(1, 2),
                            )
                            reverse_best_index = reverse_similarity.argmax(dim=-1)
                        else:
                            reverse_best_index = coarse_similarity.argmax(dim=1)
                        returned_target = reverse_best_index.gather(1, source_coarse_index)
                        current_index = torch.arange(coarse_tokens, device=input_hidden.device).view(1, -1)
                    ...
                    candidates_per_token = candidate_index.shape[-1]
                    reference_fine = key_descriptors[:, :-lag].reshape(
                        frame_pairs * batch, spatial_tokens, descriptor_dim
                    )
                    current_fine = query_descriptors[:, lag:].reshape(
            ...
            def _matched_previous_values(
                ...
                if attn_avg_mode == "c2f_value_residual_anchor":
                    ...
                    source_time = None
                elif attn_avg_mode in {"c2f_value_residual_memory", "c2f_qk_value_residual_memory"}:
                    source_index, source_time, confidence, best_similarity, reciprocal = _c2f_match_memory_indices(
                        input_grid, query_descriptors, key_descriptors
                    )
                ...
                value_grid = value.reshape(batch_size, num_tokens_t, spatial_tokens, value_channels)
                if attn_avg_mode in {"c2f_value_residual_memory", "c2f_qk_value_residual_memory"}:
            ...
            class _CorrespondenceKVProcessor:
                ...
                def __call__(
                    ...
                    key_descriptors = None
                    if attn_avg_mode == "c2f_qk_value_residual_memory":
                        query_descriptors, key_descriptors = _reduced_qk_descriptors(
                            attn.norm_q(query), attn.norm_k(key)
                        )
                    matched_values = None
                    ...
                    if attn_avg_mode in {"query_match_prev", "key_match_prev", "kv_match_prev"}:
                        ...
                        "c2f_value_residual_memory", "c2f_qk_value_residual_memory",
                    }:
                        (
                            matched_values,
                            match_confidence,
