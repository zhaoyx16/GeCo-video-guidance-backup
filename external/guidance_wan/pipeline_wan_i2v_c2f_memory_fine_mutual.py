...
class WanImageToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def __call__(
        ...
        attn_avg_match_fine_mutual: bool = False,
        ...
        Args:
            ...
            attn_avg_match_fine_mutual (`bool`, defaults to `False`):
                Require the selected fine-scale source token to map back to the target inside its local window.
                This is stricter than coarse-level reciprocity and rejects fine-scale aliases in repeated textures.
        ...
        if attn_avg_alpha > 0.0 and attn_avg_layers:
            ...
            def _c2f_match_memory_indices(input_hidden: torch.Tensor):
                ...
                offsets_x = offsets_x.reshape(1, 1, 1, 1, -1)
                target_y = (
                    torch.arange(height_tokens, device=input_hidden.device).view(1, 1, height_tokens, 1, 1)
                    + offsets_y
                ).clamp(0, height_tokens - 1)
                target_x = (
                    torch.arange(width_tokens, device=input_hidden.device).view(1, 1, 1, width_tokens, 1)
                    + offsets_x
                ).clamp(0, width_tokens - 1)
                reverse_candidate_index = (target_y * width_tokens + target_x).reshape(
                ...
                for lag in range(1, max_lag + 1):
                    ...
                    if attn_avg_match_fine_mutual:
                        selected_reference = reference_fine.gather(
                            1,
                            source_index.reshape(frame_pairs * batch, spatial_tokens)
                            .unsqueeze(-1)
                            .expand(-1, -1, descriptor_dim),
                        ).reshape(batch, frame_pairs, spatial_tokens, descriptor_dim)
                        reverse_indices = reverse_candidate_index.expand(batch, frame_pairs, -1, -1)
                        gathered_current = current_fine.reshape(
                            frame_pairs * batch, spatial_tokens, descriptor_dim
                        ).gather(
                            1,
                            reverse_indices.reshape(frame_pairs * batch, spatial_tokens * candidates_per_token)
                            .unsqueeze(-1)
                            .expand(-1, -1, descriptor_dim),
                        ).reshape(batch, frame_pairs, spatial_tokens, candidates_per_token, descriptor_dim)
                        reverse_similarity = (selected_reference.unsqueeze(-2) * gathered_current).sum(dim=-1)
                        reverse_best_index = reverse_similarity.argmax(dim=-1)
                        returned_target = reverse_indices.gather(
                            -1, reverse_best_index.unsqueeze(-1)
                        ).squeeze(-1)
                        target_index = torch.arange(spatial_tokens, device=input_hidden.device).view(1, 1, -1)
                        reciprocal_fine = returned_target.eq(target_index)

                    if reciprocal_coarse is not None:
                        coarse_reciprocal_fine = reciprocal_coarse.repeat_interleave(
                            factor, dim=2
                        ).repeat_interleave(factor, dim=3)
                        coarse_reciprocal_fine = coarse_reciprocal_fine.reshape(batch, frame_pairs, spatial_tokens)
                        reciprocal_fine = (
                            coarse_reciprocal_fine
                            if reciprocal_fine is None
                            else reciprocal_fine & coarse_reciprocal_fine
                        )
                ...
                reciprocal = best_reciprocal.reshape(
                    batch, num_tokens_t - 1, height_tokens, width_tokens
                ) if attn_avg_match_mutual or attn_avg_match_fine_mutual else None
                return (
                    best_source_index,
                    best_source_time,
