...
class WanImageToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def __call__(
        ...
        if attn_avg_alpha > 0.0 and attn_avg_layers:
            ...
            def _attention_feature_average_hook(module, inputs, output):
                if output is None:
                    return output
                if output.ndim != 3:
                ...
                hidden = output.reshape(batch_size, num_frames_tokens, height_tokens, width_tokens, channels)
                mean_feat = hidden.mean(dim=1, keepdim=True)
                hidden = (1.0 - attn_avg_alpha) * hidden + attn_avg_alpha * mean_feat
                return hidden.reshape(batch_size, seq_len, channels)
            ...
            for model in (self.transformer, self.transformer_2):
                ...
                for layer_idx in attn_avg_layers:
                    ...
                    handle = model.blocks[layer_idx].attn1.register_forward_hook(_attention_feature_average_hook)
