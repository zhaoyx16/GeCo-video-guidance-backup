...
from geometry_selection.c2f_gate import (
    geometry_validation_gate,
    prepare_geometry_bundle,
    uniform_update_scale,
)
...
class WanImageToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def __call__(
        ...
        if c2f_geometry_mode not in {"none", "hard_gate", "uniform_norm_control"}:
            raise ValueError(
                "c2f_geometry_mode must be one of: none, hard_gate, uniform_norm_control"
            )
        if c2f_geometry_mode != "none":
            ...
            if c2f_geometry_bundle is None:
                raise ValueError("c2f_geometry_bundle is required when geometry validation is enabled")
        ...
        if (attn_avg_alpha > 0.0 or c2f_diagnostics) and attn_avg_layers:
            ...
            if c2f_geometry_mode != "none":
                ...
                self._last_c2f_geometry_diagnostics = {
                    "schema": "wan-c2f-external-geometry-intervention-v1",
            ...
            class _CorrespondenceKVProcessor:
                ...
                def __call__(
                    ...
                    if matched_values is not None and match_confidence is not None:
                        ...
                        blend = base_blend
                        if c2f_geometry_mode != "none":
                            if match_source_index is None or match_source_time is None or prepared_c2f_geometry is None:
                                raise RuntimeError("geometry-gated C2F requires explicit memory source indices and times")
                            geometry_gate, geometry_info = geometry_validation_gate(
                                match_source_index,
                                match_source_time,
                                token_grid,
                            ...
                            uniform_scale, update_norms = uniform_update_scale(
                                residual,
                                base_blend,
                                geometry_gate,
                            )
                            if c2f_geometry_mode == "hard_gate":
                                blend = base_blend * geometry_gate
                                actual_update_norm = update_norms["gated_update_norm"]
                            else:
                                blend = base_blend * uniform_scale
                                actual_update_norm = update_norms["uniform_update_norm"]

                            active = match_confidence.reshape(
                                batch_size, num_tokens_t - 1, spatial_tokens
                            ) > 0

                            def _active_count(name: str) -> int:
                                return int((geometry_info[name] & active).sum().item())
                            ...
                            self._last_c2f_geometry_diagnostics["records"].append(
                                {
                                    ...
                                    "branch": attn_avg_state["branch"],
                                    "active_correspondences": active_count,
                                    "accepted_correspondences": accepted_count,
                                    "survival_fraction": accepted_count / active_count if active_count else 0.0,
                                    "rejected_reprojection": _active_count("rejected_reprojection"),
                                    "rejected_front_conflict": _active_count("front_conflict"),
                                    "abstained_occluded": _active_count("occluded"),
                                    "abstained_other": _active_count("abstained") - _active_count("occluded"),
                                    "uniform_scale": float(uniform_scale.item()),
                                    "base_update_norm": float(update_norms["base_update_norm"].item()),
                                    "gated_update_norm": float(gated_norm.item()),
