...
class WanImageToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def __call__(
        ...
        latent_consist_capture = {"enabled": False, "features": None}
        latent_consist_active = latent_consist_lr > 0.0
        if latent_consist_active:
            p_t, p_h, p_w = patch_size
            token_grid = (latents.shape[2] // p_t, latents.shape[3] // p_h, latents.shape[4] // p_w)

            # Freeze weights but keep the input-gradient path through the transformer.
            self.transformer.requires_grad_(False)
            if getattr(self, "transformer_2", None) is not None:
                self.transformer_2.requires_grad_(False)

            def _latent_consist_capture_hook(module, inputs, output):
            ...
            for model in (self.transformer, self.transformer_2):
                ...
                handle = model.blocks[latent_consist_layer].attn1.register_forward_hook(_latent_consist_capture_hook)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                do_latent_consist_guidance = (
                    latent_consist_active
                    and latent_consist_start_step <= i <= latent_consist_end_step
                )
                ...
                if do_latent_consist_guidance:
                    ...
                    with torch.enable_grad():
                        ...
                        if num_frames_tokens > 1:
                            loss = ((features_5d[:, 1:, :, :, :] - features_5d[:, :-1, :, :, :]) ** 2).sum()
                        else:
                        ...
                        print(f"latent_consist_loss({i}): {loss_value:.6f}", flush=True)

                        latents = (latents - latent_consist_lr * grad).detach()
                        noise_pred = noise_pred.detach()
                        latent_model_input = latent_model_input.detach()
                        del grad, loss, features, features_5d
                else:
                    if self.config.expand_timesteps:
                        latent_model_input = (1 - first_frame_mask) * condition + first_frame_mask * latents
                        latent_model_input = latent_model_input.to(transformer_dtype)
