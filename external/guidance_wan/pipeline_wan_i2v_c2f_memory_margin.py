...
class WanImageToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def __call__(
        ...
        attn_avg_match_margin: float = 0.0,
        ...
        Args:
            ...
            attn_avg_match_margin (`float`, defaults to `0.0`):
                Minimum coarse-match margin between the best and second-best source for full memory blending.
                A positive value softly suppresses ambiguous repeated-texture correspondences.
        ...
        if attn_avg_match_margin < 0.0:
            raise ValueError("attn_avg_match_margin must be non-negative")
        ...
        if attn_avg_alpha > 0.0 and attn_avg_layers:
            ...
            def _c2f_match_memory_indices(input_hidden: torch.Tensor):
                ...
                best_margin = torch.zeros_like(best_confidence)
                ...
                for lag in range(1, max_lag + 1):
                    ...
                    coarse_top2_similarity, coarse_top2_index = coarse_similarity.topk(k=2, dim=-1)
                    source_coarse_index = coarse_top2_index[..., 0]
                    coarse_margin = (
                        coarse_top2_similarity[..., 0] - coarse_top2_similarity[..., 1]
                    ...
                    fine_margin = coarse_margin.repeat_interleave(factor, dim=2).repeat_interleave(
                        factor, dim=3
                    ).reshape(batch, frame_pairs, spatial_tokens)
                    if attn_avg_match_margin > 0.0:
                        confidence = confidence * (fine_margin / attn_avg_match_margin).clamp(0.0, 1.0)
                    ...
                    best_margin[:, target_slots] = torch.where(
                        replace, fine_margin, best_margin[:, target_slots]
                    )
                ...
                return (
                    ...
                    reciprocal,
                    best_margin.reshape(batch, num_tokens_t - 1, height_tokens, width_tokens),
                )
                    ...
                    if attn_avg_debug and self.layer_idx == attn_avg_layers[0] and match_similarity is not None:
                        ...
                        if print_key not in attn_avg_state["printed"]:
                            ...
                            if match_margin is not None:
                                margin_flat = match_margin.float().reshape(-1)
                                margin_info = (
                                    f"coarse_margin_mean={margin_flat.mean().item():.3f} "
                                    f"coarse_margin_p10={torch.quantile(margin_flat, 0.1).item():.3f} "
                                    f"coarse_margin_p90={torch.quantile(margin_flat, 0.9).item():.3f} "
                                )
                            displacement_info = ""
