...
class WanImageToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def __call__(
        ...
        if geometry_transport_mode not in {
            ...
            raise ValueError(
                "geometry_transport_mode must be shadow, value_residual, kv_memory, "
                "kv_transport, or qkv_transport"
            )
        ...
        if (attn_avg_alpha > 0.0 and attn_avg_layers) or geometry_transport_layers:
            ...
            if geometry_map_path is not None:
                ...
                confidence = map_payload["confidence"].float()
                if (
                    source_time.shape[:2] != (expected_temporal, expected_spatial)
                    or source_index.shape != source_time.shape
                    or confidence.shape[:3] != (
                        expected_temporal,
                        token_grid[1],
                        token_grid[2],
                    )
                    or confidence.shape[-1] != source_time.shape[-1]
                ):
                    raise ValueError(
                ...
                valid_fraction = (confidence.amax(dim=-1) > 0).float().mean().item()
                print(
                    f"[geometry-transport] loaded={geometry_map_path} "
                    f"coverage={valid_fraction:.3f} slots={source_time.shape[-1]}",
                    flush=True,
                )
