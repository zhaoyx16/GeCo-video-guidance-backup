...
def main() -> None:
    ...
    coverage = float(transport.confidence.amax(dim=-1).gt(0).float().mean().item())
    report = {
        ...
        "map_coverage_all_target_tokens": coverage,
        "confidence_percentile": args.confidence_percentile,
        "confidence_floor": args.confidence_floor,
        "visibility_mode": args.visibility_mode,
        "behind_threshold": args.behind_threshold,
        ...
        "interpretation": (
            ...
            "front_tolerant visibility permits an early surface to repair a "
            "surface that receded or disappeared in the baseline draft while "
            "still rejecting anchors behind a target occluder."
        ),
    }
