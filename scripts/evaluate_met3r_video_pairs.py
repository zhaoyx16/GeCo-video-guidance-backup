...
def main() -> None:
    ...
    print(
        "metric=MEt3R backbone=mast3r feature=dino_vits16 "
        "upsampler=bilinear distance=cosine img_size=None",
        flush=True,
    )
    ...
    metric = MEt3R(
        ...
        use_norm=True,
        backbone="mast3r",
        feature_backbone="dino_vits16",
        feature_backbone_weights="facebookresearch/dino:main",
        upsampler="bilinear",
        distance="cosine",
        freeze=True,
    )
    # This is the same token reshape and ImageNet normalization used by
    # FeatUp's DINOFeaturizer. Only the learned FeatUp upsampler is replaced
    # by MEt3R's supported bilinear interpolation mode.
