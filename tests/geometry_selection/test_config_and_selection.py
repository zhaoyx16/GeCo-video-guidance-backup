...
from geometry_selection.scorer import GeometryScoreReport, ScorerConfig
...
def test_all_invalid_candidates_retain_incumbent() -> None:
    candidates = [
        CandidateScore("candidate0", 0, "a" * 64, True, report(float("inf"), 0.0, "no_valid_local_edges")),
        CandidateScore("candidate1", 1, "b" * 64, False, report(float("inf"), 0.0, "no_valid_local_edges")),
    ]
    result = select_candidate(candidates, SelectionConfig())
    assert result.selected_candidate_id == "candidate0"
    assert result.decision == "abstain_no_valid_candidate"
