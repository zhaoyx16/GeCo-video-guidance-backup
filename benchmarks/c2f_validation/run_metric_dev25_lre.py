#!/usr/bin/env python3
"""Run sealed Independent LRE with the paired dev25 eligibility contract."""
...
def main() -> None:
    ...
    parser.add_argument("metric", choices=["long_range_reprojection_error"])
    ...
    if metric == "long_range_reprojection_error" and lock_payload.get("method_id") != "wan_unguided_seed0":
        binding = lock_payload.get("baseline_metric_eligibility_lock")
        if not isinstance(binding, dict) or not isinstance(binding.get("path"), str):
            raise ValueError("compared LRE input lock lacks baseline eligibility binding")
        eligibility_path = base.require_readonly(Path(binding["path"]), binding.get("sha256"))
        eligibility = json.loads(eligibility_path.read_text(encoding="utf-8"))
        locked_units = eligibility.get("locked_units", {}).get("long_range_reprojection_error")
        entry_case_ids = {entry["case_id"] for entry in entries}
        if (
            eligibility.get("schema") != "geometry-selection-metric-eligibility-lock-v1"
            or not isinstance(locked_units, list)
            or not locked_units
            or any(
                not isinstance(unit, dict)
                or unit.get("unit_id") != "first_last"
                or unit.get("case_id") not in entry_case_ids
                for unit in locked_units
            )
            or len({unit["case_id"] for unit in locked_units}) != len(locked_units)
        ):
            raise ValueError("compared LRE eligibility binding is malformed")
        expected_records = len(locked_units)
    if (
