#!/usr/bin/env python3
"""Run one sealed evaluator-v13 metric on an immutable 10-case Stage B lock."""
...
ADAPTER_HELPERS = HERE.parent / "c2f_validation"
...
def main() -> None:
    ...
    supported_metrics = sorted(set(metrics) - {"long_range_reprojection_error"})
    ...
    if args.expected_entries != 10:
        raise ValueError("this launcher is frozen for the 10-case Stage B experiment")
    ...
    if lock_payload.get("scope") != "stage_b_development_subset_disjoint_from_frozen_test_validation_debug":
        raise ValueError("input lock is not the frozen disjoint Stage B scope")
    ...
    if metric in custom_adapter_names:
        adapter_source = ADAPTER_HELPERS / custom_adapter_names[metric]
    ...
    receipt = {
        "schema": "geometry-selection-evaluator-v13-disjoint-stage-b-component-run-v1",
        "status": "complete",
        "metric_id": metric,
