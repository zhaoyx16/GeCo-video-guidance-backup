...
WINDOW_BUNDLE_SCHEMA = "geometry-window-bundle-v1"
...
def independent_run_id(
    *,
    ...
    return canonical_hash(
        {
            "logical_inference": "separate-vggt-omega-window-forward-v1",
...
def validate_window_bundle(
    ...
    *,
    ...
    if (
        ...
        or len(keyframes) < 4
        or any(isinstance(value, bool) or not isinstance(value, int) for value in keyframes)
    ...
    for record in windows:
        ...
        digests = [record["geometry_cache_key"], record["independent_run_id"], *pixels]
        if any(
            ...
            raise ValueError("window cache/run/pixel identities must be SHA-256 digests")
        ...
        if kind == "local":
            ...
            loop_count += 1
        schedule.append({"window_id": identifier, "kind": kind, "frame_indices": frames})
    ...
    if canonical_hash({"windows": schedule}) != bundle["schedule_sha256"]:
